...
{
 Properties { _Color("Tint",Color)=(1,1,1,1) _MainTex("Albedo",2D)="white"{} _Kind("Skin leather satin canvas",Float)=0 }
 SubShader { Tags { "RenderType"="Opaque" } Pass {
 CGPROGRAM
 #pragma vertex vert
 #pragma fragment frag
 #include "UnityCG.cginc"
 sampler2D _MainTex; float4 _Color,_MainTex_ST; float _Kind;
 struct A { float4 vertex:POSITION; float3 normal:NORMAL; float2 uv:TEXCOORD0; };
 struct V { float4 pos:SV_POSITION; float3 n:TEXCOORD0; float3 view:TEXCOORD1; float2 uv:TEXCOORD2; float3 world:TEXCOORD3; };
 V vert(A a) { V o; o.pos=UnityObjectToClipPos(a.vertex); o.n=UnityObjectToWorldNormal(a.normal); o.view=WorldSpaceViewDir(a.vertex); o.uv=a.uv; o.world=mul(unity_ObjectToWorld,a.vertex).xyz; return o; }
 ...
 float3 albedo=tex2D(_MainTex,i.uv*_MainTex_ST.xy+_MainTex_ST.zw).rgb*_Color.rgb;
 if(_Kind>4.5&&_Kind<5.5)albedo=tex2D(_MainTex,float2(frac(i.uv.x),.87+frac(i.uv.y)*.12)).rgb*_Color.rgb;
 if(_Kind>5.5){float seam=step(.70,abs(sin(i.uv.y*110+i.uv.x*5)));albedo=lerp(albedo,albedo*float3(.6,.13,.13),seam*.45);}
