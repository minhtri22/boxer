...
namespace BoxerP0
{
    ...
    public sealed class EVVisualShell : MonoBehaviour
    {
        ...
        public static bool AnatomicalExperiment=true;
        ...
        void BuildOpponent(Transform root)
        {
            ...
            if(!AnatomicalExperiment)Replace(head,SphereMesh(64,48),_head);
            ...
            if(!AnatomicalExperiment&&!SegmentedExperiment)
            {
            ...
            else if(!AnatomicalExperiment)foreach(string n in new[]{"R2 Abdomen","R2 Chest","R2 Left Chest","R2 Right Chest","R2 Neck"}) root.Find(n).GetComponent<Renderer>().sharedMaterial=_skin;
            if(AnatomicalExperiment) { if(_body!=null) _body.GetComponent<Renderer>().enabled=false; var skin=new GameObject("EV Ramirez Anatomical Skin").AddComponent<EVAnatomy>();skin.Initialize(root,_skin,_head); }
            ...
            foreach(string side in new[]{"Left","Right"})
            {
                ...
                var boot=MeshObject("EV "+side+" Boot Upper",shin,Loft(new[]{new Vector3(0,-.15f,0),new Vector3(.86f,-.15f,.78f),new Vector3(.90f,.45f,.85f),new Vector3(.70f,1,.65f)},24),_white);
                if(AnatomicalExperiment){boot.GetComponent<Renderer>().enabled=false;continue;} // replaced by the fitted skinned boot surface
                // Crossed laces are local details on the same shin; not independent foot writers.
                for(int j=0;j<5;j++)
                {
                    float y=-.06f+j*.20f;
                    Line("Lace",boot,new Vector3(-.48f,y,.78f),new Vector3(.48f,y+.13f,.78f),.024f,_red);
                    Line("Lace",boot,new Vector3(.48f,y,.78f),new Vector3(-.48f,y+.13f,.78f),.024f,_red);
                }
            }
        }
        void BuildArms(Transform root,bool player)
        {
            foreach(string side in new[]{"Left","Right"})
            {
                ...
                var thumb=MeshObject("EV "+side+" Glove Thumb",cuff,SphereMesh(24,16),player?_black:_red);thumb.localPosition=new Vector3(side=="Left"?.29f:-.29f,-.29f,.12f);thumb.localScale=new Vector3(.36f,.48f,.40f);
        ...
        void Word(Transform parent,string text,Vector3 position,float size,Material color)
        {
            ...
            if(text=="RAMIREZ"){tm.characterSize=.017f;go.transform.localScale=new Vector3(1,6,1);}
        ...
        void Arena()
        {
            ...
            canvas.color=new Color(.30f,.33f,.34f);canvas.mainTexture=Resources.Load<Texture2D>("EV/GloveLeather");canvas.mainTextureScale=new Vector2(24,24);
            ...
            var backdrop=MeshObject("EV Distant Arena Scenery",root,quad,backdropMat);backdrop.position=new Vector3(0,3.0f,4.24f);backdrop.localScale=new Vector3(14f,9.506f,1);
