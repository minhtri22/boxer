...
def main():
    ...
    module = runpy.run_path(
        ...
        run_name="ramirez_glove_review19_module",
    )
    ...
    out = repo / "art" / "blender" / "ramirez" / "renders" / "review19-glove-proportion"
    ...
    head_points = [
        ...
        if 1.55 <= vertex.co.z <= 1.76
    ]
    head_width = max(point.x for point in head_points) - min(point.x for point in head_points)
    glove_width = float(left.dimensions.x)
    glove_to_head_ratio = glove_width / max(float(head_width), 1e-9)
    report = {
        "scope": "GLOVE_ONLY",
        "phase": args.phase,
        ...
        "mass_scale": round(float(left.get("mass_scale", 1.0)), 3),
        "glove_width_m": round(glove_width, 4),
        "head_width_m": round(float(head_width), 4),
        "glove_to_head_ratio": round(glove_to_head_ratio, 3),
        "reference_visual_target_glove_to_head_band": [0.95, 1.08],
        ...
        "numeric_sanity_checks": {
            ...
            "S04_thumb_geometry_present": (
                ...
                and 0.40 <= (
                    float(gloves["L"]["thumb"].dimensions.x)
                    / max(glove_width, 1e-9)
                ) <= 0.65
            ),
            ...
            "S06_glove_to_head_reference_proportion": 0.95 <= glove_to_head_ratio <= 1.08,
        },
    }
