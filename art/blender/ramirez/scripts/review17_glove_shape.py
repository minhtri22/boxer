...
def main():
    ...
    module = runpy.run_path(
        ...
        run_name="ramirez_glove_review17_module",
    )
    ...
    out = repo / "art" / "blender" / "ramirez" / "renders" / "review17-glove-shape"
    ...
    report = {
        ...
        "max_forward_ring_z_m": round(float(left.get("max_forward_ring_z_m", 0.0)), 4),
        "striking_face_half_width_m": round(float(left.get("striking_face_half_width_m", 0.0)), 4),
        "underside_pole_recess_m": round(float(left.get("underside_pole_recess_m", 0.0)), 4),
        ...
        "automated_geometry_checks": {
            "G01_front_tip_removed": (
                left.get("distal_terminal") == "underside_pole_behind_round_striking_face"
                and float(left.get("underside_pole_recess_m", 0.0)) >= 0.025
                and float(left.get("terminal_ring_half_width_m", 0.0)) >= 0.035
                and float(left.get("striking_face_half_width_m", 0.0)) >= 0.070
            ),
            "G02_clenched_fist_mass": (
                left.get("shape_profile") == "rounded_curled_fist_v17"
                and float(left.get("knuckle_half_width_m", 0.0))
                / max(float(left.get("wrist_half_width_m", 1.0)), 1e-9) >= 1.8
            ),
            "G03_front_curls_downward": float(left.get("fist_curl_drop_m", 0.0)) >= 0.15,
            "G04_thumb_bulge_visible": (
                gloves["L"].get("thumb") is not None
                and float(gloves["L"]["thumb"].dimensions.x) >= 0.045
            ),
            "G05_white_cuff_visible": (
    ...
    (out / "automated-glove-shape-precheck.json").write_text(
